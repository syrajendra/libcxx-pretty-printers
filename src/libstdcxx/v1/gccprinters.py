...
fd = None
version = "1.0.0"
# enable debug log if LIBCXX_PREETY_PRINT_DEBUG is set
if 'LIBCXX_PREETY_PRINT_DEBUG' in os.environ:
    try:
        import getpass
        from inspect import currentframe, getframeinfo,stack
        username = getpass.getuser()
        log_file = '/tmp/'+ username + '-pretty-printer.log'
        fd = open(log_file, "w")
        fd.write("#######################################################################\n")
        fd.write("#                      Pretty-printer log                             #\n")
        fd.write("#                         Version: %4s                              #\n"%(version))
        fd.write("#######################################################################\n")
        fd.flush()
    except IOError as e:
...
'''
 Writes message to /tmp/<username>-pretty-printer.log file
 It does follow following format
 filename : class:function:linenumber : message
 Example :
 printers.py : libcxx.v1.printers.RbtreeIterator:  __next__ :  637  => node: 0x55555557f3d0  parent_addreess :0x55555557f240
'''

def LOG_MSG(msg):
    if fd:
        lines  = msg.split('\n')
        s = currentframe().f_back
        filename = s.f_code.co_filename
        class_ = str(s.f_locals["self"].__class__)
        function = s.f_code.co_name
        lineno = s.f_lineno
        fd.write("%10s : %25s:%10s : %4s  => %s\n" %(os.path.basename(filename), class_, function, lineno, lines[0]))
        for line in lines[1:]:
            fd.write("%68s %s\n" %("", line))
            fd.flush()
...
class Tr1UnorderedSetPrinter:
    ...
    def hashtable (self):
        if self.typename.startswith('std::tr1')  or\
            self.typename.startswith('std::unordered_set'):
            return self.val
    ...
    def to_string (self):
        try:
            ...
            count = self.hashtable()['_M_h']['_M_element_count']

        return '%s with %d elements' % (self.typename, count)
    ...
    def children (self):
        ...
        if self.typename.startswith('std::tr1') or\
           self.typename.startswith('std::unordered_set'):
           try:
               return izip (counter, StdHashtableIterator (self.hashtable()['_M_h']))
           except Exception:
...
class Tr1UnorderedMapPrinter:
    ...
    def hashtable (self):
        if self.typename.startswith('std::tr1') or\
            self.typename.startswith('std::unordered_map'):
            return self.val
        return self.val['_M_h']
        ...
        if self.typename.startswith('std::tr1') or\
           self.typename.startswith('std::unordered_map'):
            try:
                ...
                data = self.flatten (imap (self.format_one, StdHashtableIterator (self.hashtable()['_M_h'])))
                # Zip the two iterators together.
                return izip (counter, data)
...
class StdForwardListPrinter:
    ...
    class _iterator(Iterator):
        def __init__(self, nodetype, head):
            self.nodetype = nodetype
            self.base = head['_M_next']
            LOG_MSG("base for storage :%s"%self.base)
            self.count = 0
